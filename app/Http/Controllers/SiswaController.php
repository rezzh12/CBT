...
class SiswaController extends Controller
{
    ...
    public function detailSiswa(Request $request)
    {
      if (auth()->user()->status == 'G' or auth()->user()->status == 'A') {
        ...
        ->select('soals.paket', DB::raw('SUM(jawabs.score) as jumlah_nilai'))
        ->where('jawabs.id_user', $siswa->id)
        ->where('soals.id_user', $user->id)
        ->groupBy('jawabs.id_soal', 'soals.paket')  // Tanpa jawabs.id
    ...
    public function detailUjian($id)
    {
      ...
      if ($check_soal) {
        ...
        ->where('status', 'Y')
        ->inRandomOrder()  // Mengacak urutan soal
        ->get();
        $waktu = Soal::where('id', $id)->value('waktu'); 
        $waktu_mulai = Carbon::now();  // Waktu sekarang
          $waktu_berakhir = $waktu_mulai->addMinutes($waktu);  // Menambahkan durasi ujian ke waktu sekarang
  
          $waktu_sekarang = Carbon::now();  // Waktu sekarang
  
          // Hitung selisih waktu dalam detik
          $selisih_detik = $waktu_berakhir->diffInSeconds($waktu_sekarang);
        return view('halaman-siswa.detail_ujian', compact('soal', 'soals','selisih_detik'));
      } else {
        return redirect()->route('home.index');
      }
    }
