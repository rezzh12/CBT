...
class SoalController extends Controller
{
    ...
    public function detail(Request $request)
    {
      if (Auth::user()->status == 'G' or Auth::user()->status == 'A') {
        ...
        if (count($cekDistribusisoal) > 0) {
          ...
          ->select('distribusisoals.id_soal', 'kelas.id', 'kelas.nama', DB::raw('COUNT(kelas.id) as kelas_count'))
          ->groupBy('distribusisoals.id_soal', 'kelas.id', 'kelas.nama') // Group by relevant columns
    ...
    public function dataDetailSoal(Request $req)
    {
      $id_soal = $req->input('id');  // Get 'id' from the request
      
      // Fetch the details based on 'id_soal'
      $soals = Detailsoal::where('id_soal', $id_soal)->get();
    
      return Datatables::of($soals)
        ->editColumn('status', function ($soals) {
          return $soals->status == 'Y' ? "<center><span class='label label-success'>Tampil</span></center>" : "<center><span class='label label-danger'>Tidak tampil</span></center>";
        })
        ...
        ->addColumn('action', function ($soals) {
          return "<div style='text-align:center'><a href='ubah/{$soals->id}' class='btn btn-success btn-xs'>Ubah</a> <a href='data-soal/{$soals->id}' class='btn btn-primary btn-xs'>Detail</a> <a href='hapus/{$soals->id}' class='btn btn-danger btn-xs'>Hapus</a> </div>";
        })
    ...
    public function hapusDetailSoal(Request $request)
    {
        // Check if the user has the correct status ('G' or 'A')
        if (Auth::user()->status == 'G' or Auth::user()->status == 'A') {
            // Get the ID of the soal from the request
            $id_soal = $request->id;
            
            // Fetch the authenticated user
            $user = User::where('id', Auth::user()->id)->first();
            
            // Find the detail soal by its ID
            $soal = Detailsoal::where('id', $id_soal)->first();
            ...
            if ($soal) {
                ...
                // Redirect to the previous page or a specific route with success message
                return redirect()->back()->with('success', 'Detail soal berhasil dihapus');
            } else {
                // If soal not found, return with error message
                return redirect()->back()->with('error', 'Detail soal tidak ditemukan');
            }
        } else {
