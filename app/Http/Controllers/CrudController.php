...
class CrudController extends Controller
{
    ...
    {
      Kelas::where('id',$request->id_kelas)->delete();
      return 1;
    }
    public function simpanDistribusi(Request $request)
    {
      ...
      if ($request->id == 'N') {
        ...
        $query = Distribusisoal::find($request->id);
      }
      $query->id_soal = $request->id_soal;
      $query->id_kelas = $request->id_kelas;
      if ($query->save()) {
    ...
    {
      Distribusisoal::where('id',$id)->delete();
...
{       
  ...
  return redirect()->route('master.siswa')->with($notification);
}
