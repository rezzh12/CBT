...
<div class="col-md-8">
  <div class="box box-primary">
    ...
    <div class="box-body">
      ...
      <a class="pull-right" href="{{ url('/soal-export') }}">Download format Excel</a>
      ...
      <div class="well" style="margin-top: 15px; display: none;" id="wrap-upload-soal">
        <form class="form-horizontal" action="{{ url('/soal-import') }}" enctype="multipart/form-data" method="POST">
          {{ csrf_field() }}
          <div class="box-body">
            <div class="form-group">
...
<script>
  $(document).ready(function() {
    ...
    $(document).ready(function() {
  var id = window.location.pathname.split('/').pop();  // Get the last segment of the URL
  console.log(id);  // Check if id is correctly fetched
  ...
  table_soal = $('#table_detail_soal').DataTable({
    ...
    lengthChange: true,
    ajax: {
      url: "{{ route('elearning.get-detail-soal') }}",  // Correct usage of route
      data: function(d) {
        d.id = id;  // Send the 'id' as part of the request data
      }
    },
    columns: [
    ...
    drawCallback: function(settings) {
      // You can add additional behavior when the table is redrawn (e.g., after sorting or filtering)
    }
  });
    ...
    let table_soal_essay = $("#table_detail_soal_essay").DataTable({
      ...
      lengthChange: true,
      ajax: {
        url: "{{ route('elearning.soalessay.data') }}",
        data: function(d) {
          d.id_soal = "{{ $soal->id }}"
        }
      },
