...
$(document).ready(function () {
  table_materi = $('#table_materi').DataTable({
    ...
    responsive: true,
    lengthChange: true,
    ajax: '{{ route("elearning.dataMateriGuru") }}',  // Fixed the route helper issue
    columns: [
    ...
    drawCallback: function(settings) {
      // You can implement any custom functionality here if needed
    }
  });
  ...
  // Toggle the materi section
  $("#btn-materi").click(function() {
    $("#wrap-materi").slideToggle();
  });

  // Hide materi section when cancel is clicked
  $("#batal").click(function() {
    $("#wrap-materi").slideToggle();
  });

  // Handle the form submission for saving materi
  $("#simpan").click(function(e) {
    e.preventDefault();  // Prevent the default form submit
    $("#loading").show();  // Show the loading indicator

    var dataString = $("#formMateri").serialize();  // Serialize the form data
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (data == 'ok') {
          ...
          window.location = "{{ url('/elearning/materi') }}";  // Redirect after successful save
        } else {
          $("#loading").hide();
          $("#wrap-btn").show();
          $("#notif").removeClass('alert alert-info').addClass('alert alert-danger').html(data).fadeIn(350);  // Show error message
        }
      }
    });
  });
